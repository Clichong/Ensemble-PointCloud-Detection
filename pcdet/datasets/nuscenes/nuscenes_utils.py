...
def get_available_scenes(nusc):
    ...
    for scene in nusc.scene:
        ...
        sample_rec = nusc.get('sample', scene_rec['first_sample_token'])    # first sample in scene
        sd_rec = nusc.get('sample_data', sample_rec['data']['LIDAR_TOP'])   # first sample lidar data in scene
...
def fill_trainval_infos(data_path, nusc, train_scenes, val_scenes, test=False, max_sweeps=10, with_cam=False):
    ...
    for index, sample in enumerate(nusc.sample):    # len(nusc.sample): 404
        progress_bar.update()
        ...
        sweeps = []
        while len(sweeps) < max_sweeps - 1:     # 挑选sweep（不超过max_sweeps）
            if curr_sd_rec['prev'] == '':
                if len(sweeps) == 0:
                    sweep = {
                        'lidar_path': Path(ref_lidar_path).relative_to(data_path).__str__(),
...
def lidar_nusc_box_to_global(nusc, boxes, sample_token):
    '''
        对激光雷达坐标系转到车身坐标系，再转换到全局坐标系
    '''
    s_record = nusc.get('sample', sample_token)
    sample_data_token = s_record['data']['LIDAR_TOP']   # 使用激光雷达传感器的calibrated_sensor以及ego_pose
...
def transform_det_annos_to_nusc_annos(det_annos, nusc):
    ...
    for det in det_annos:
        ...
        box_list = lidar_nusc_box_to_global(        # 坐标系的转换
            nusc=nusc, boxes=box_list, sample_token=det['metadata']['token']
        )

        for k, box in enumerate(box_list):
            name = det['name'][k]
            if np.sqrt(box.velocity[0] ** 2 + box.velocity[1] ** 2) > 0.2:    # 速度判断类别当前属性
...
def format_nuscene_results(metrics, class_names, version='default'):
    ...
    for name in class_names:
        ...
        # show every class each aps and error
