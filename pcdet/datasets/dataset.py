...
class DatasetTemplate(torch_data.Dataset):
    ...
    def generate_prediction_dicts(self, batch_dict, pred_dicts, class_names, output_path=None):
        ...
        for index, box_dict in enumerate(pred_dicts):   # process frame
            ...
            if 'metadata' in batch_dict:
                single_pred_dict['metadata'] = batch_dict['metadata'][index]    # add token info
    ...
    def prepare_data(self, data_dict):
        ...
        data_dict = self.set_lidar_aug_matrix(data_dict)    # 构建数据增强变换矩阵在data_dict中
        if data_dict.get('gt_boxes', None) is not None:
            ...
            data_dict['gt_boxes'] = gt_boxes    # 筛选不在设置中的类别，然后添加类别标签在gt的最后一维中
        ...
        if self.training and len(data_dict['gt_boxes']) == 0:   # 当前的scene没有gt，则用另外的scene来对当前的进行替换
            new_index = np.random.randint(self.__len__())
            return self.__getitem__(new_index)

        data_dict.pop('gt_names', None)    # 类别信息已经添加到gt的一个维度中

        return data_dict

    @staticmethod
    def collate_batch(batch_list, _unused=False):
        data_dict = defaultdict(list)   # 初始化为列表结构，直接在后续中进行append添加
            ...
            try:
                if key in ['voxels', 'voxel_num_points']:
                    ...
                    for i, coor in enumerate(val):
                        coor_pad = np.pad(coor, ((0, 0), (1, 0)), mode='constant', constant_values=i)   # 在首列填充场景编号
